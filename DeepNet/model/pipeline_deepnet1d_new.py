...
def _build_generic_deepnet_pipeline_1d(module_cls, num_encoder_layers, num_decoder_layers, num_chunks, device=torch.device('cuda'), **kwargs):
    if num_encoder_layers == 0: #decoder-only
        ...
        return model
    elif num_encoder_layers>0 and num_decoder_layers>0: #encoder-decoder
        #TODO
        return None



def _build_deepnet_pipeline_1d(num_encoder_layers, num_decoder_layers, num_chunks, device=torch.device('cuda'), fused=False, **kwargs):
    model = FusedPipelineDeepNet1D if fused else PipelineDeepNet1D
    return _build_generic_deepnet_pipeline_1d(model, num_encoder_layers, num_decoder_layers, num_chunks, device, **kwargs)
...
def deepnet_small_pipeline_1D(num_chunks=1, checkpoint=False, dtype=torch.float, embed_split_hidden=False, fused=False):
    ...
    return _build_deepnet_pipeline_1d(0, 12, num_chunks, fused=fused, **cfg)
